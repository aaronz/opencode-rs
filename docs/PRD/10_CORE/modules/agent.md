...
> **User Documentation**: [agents.mdx](https://github.com/anomalyco/opencode/blob/main/packages/web/src/content/docs/zh-cn/agents.mdx) — Overview of agent types and usage

**See Also**: [Glossary: Agent](../../system/01_glossary.md#agent) | [System PRD: Agent System](../../system/02-agent-system.md)
...
pub enum DelegationError {
    ...
    TaskAlreadyCompleted { task_id: TaskId },
...
|-------|-------------|--------------|-------------|
| `Pending` | Task created, waiting to start | Initialize task metadata | Assign to worker |
| `Running` | Task actively being executed | Update `updated_at` | Record result or error |
| `Completed` | Task completed successfully | Store `TaskResult` | Mark task as done |
| `Failed` | Task execution failed | Store error reason | Enable retry |
| `Cancelled` | Task cancelled by user | Clear pending work | Stop execution |

### Task State Transitions

| From | To | Trigger | Validation |
|------|-----|---------|------------|
| - | Pending | `create_task()` | Valid task description |
| Pending | Running | `start()` | Worker available |
| Pending | Cancelled | `cancel()` | Task not started |
| Running | Completed | `complete(result)` | Valid result |
| Running | Failed | `fail(error)` | Error occurred |
| Running | Cancelled | `cancel()` | Cleanup started work |
| Failed | Pending | `retry()` | Retry count < max_retries |
| Failed | Cancelled | `abandon()` | No more retries |
| Completed | - | `archive()` | Optional cleanup |
| Cancelled | - | `archive()` | Optional cleanup |

### Task Invariants

| Invariant | Description |
|-----------|-------------|
| Task ID is immutable | Once created, `TaskId` never changes |
| Status monotonicity | Cannot go backward (Completed → Pending invalid) |
| Result only on terminal state | `TaskResult` only set when Completed |
| Max retries on Failed | Exceeding max_retries moves to Cancelled |
...
| `TaskAlreadyCompleted` | - | Cannot modify completed task |
| `MaxRetriesExceeded` | - | Retry limit reached |
| `DelegateBusy` | - | No worker available |
| `TaskCancelled` | - | Task was cancelled |

---

## PrimaryAgentState Machine

```mermaid
stateDiagram-v2
    [*] --> Inactive: new()

    Inactive --> Running: activate(agent_type)
    Running --> Transitioning: switch_agent()
    Transitioning --> Running: complete_transition()

    Running --> Inactive: deactivate()
    Transitioning --> Running: cancel_transition()
```

### PrimaryAgentState Definitions

| State | Description | Entry Action | Exit Action |
|-------|-------------|--------------|-------------|
| `Inactive` | No primary agent active | Initialize | `activate()` |
| `Running` | Agent actively processing | Begin execution | `deactivate()` or `switch_agent()` |
| `Transitioning` | Switching between agents | Begin agent switch | `complete_transition()` or `cancel_transition()` |
...
|----|-----------|------------------|
| AC-T001 | Create task | Given valid task description, When `TaskDelegate::create_task()`, Then task created with `Pending` status |
| AC-T002 | Start task | Given `Pending` task, When `start()`, Then task becomes `Running` |
| AC-T003 | Complete task | Given `Running` task, When `complete(result)`, Then task becomes `Completed` with result |
| AC-T004 | Fail task | Given `Running` task, When `fail(error)`, Then task becomes `Failed` |
| AC-T005 | Cancel pending task | Given `Pending` task, When `cancel()`, Then task becomes `Cancelled` |
| AC-T006 | Cancel running task | Given `Running` task, When `cancel()`, Then task becomes `Cancelled`, work stopped |
| AC-T007 | Retry failed task | Given `Failed` task with retries < max, When `retry()`, Then task becomes `Pending` |
| AC-T008 | Max retries exceeded | Given `Failed` task with retries >= max, When `retry()`, Then task becomes `Cancelled` |
| AC-T009 | TaskNotFound error | Given invalid task ID, When operation, Then return `TaskNotFound` |
| AC-T010 | Cannot complete completed task | Given `Completed` task, When `complete()`, Then return `TaskAlreadyCompleted` |

### PrimaryAgentTracker

| ID | Criterion | Given-When-Then |
|----|-----------|------------------|
| AC-PAT001 | Activate first agent | Given inactive tracker, When `activate(Build)`, Then state=Running, agent=Build |
| AC-PAT002 | Cannot activate second | Given active tracker with Build, When `activate(Plan)`, Then return `MultiplePrimaryAgents` error |
| AC-PAT003 | Switch agent | Given Running with Build, When `switch_agent(Plan)`, Then state=Transitioning |
| AC-PAT004 | Complete transition | Given Transitioning, When `complete_transition(Plan)`, Then state=Running, agent=Plan |
| AC-T005 | Deactivate | Given Running, When `deactivate()`, Then state=Inactive |

### Agent Event Emission

| ID | Criterion | Test Method |
|----|-----------|-------------|
| AC-EVT001 | Events emitted on start | Given agent running, When `run_loop()` starts, Then `AgentEvent::Started` emitted |
| AC-EVT002 | Events emitted on completion | Given agent completes, When `run_loop()` ends, Then `AgentEvent::Completed` emitted |
| AC-EVT003 | Events emitted on error | Given agent errors, When error occurs, Then `AgentEvent::Error` emitted |
| AC-EVT004 | Events emitted on tool call | Given agent calls tool, When tool executes, Then `AgentEvent::ToolCall` emitted |
| AC-EVT005 | Broadcast to all subscribers | Given multiple subscribers, When event emitted, Then all receive event |
...
| Agent Type | Read | Write | Execute | Tools | Subagent |
|------------|------|-------|--------|-------|----------|
| Build | ✓ | ✓ | ✓ | Full | ✓ |
| Plan | ✓ | ✗ | ✗ | None | ✗ |
| General | ✓ | ✗ | ✗ | Limited | ✗ |
...
| Refactor | ✓ | ✓ | ✗ | Refactor | ✗ |
| Review | ✓ | ✗ | ✗ | Review | ✗ |
| Compaction | System agent — not user visible | | | | |
| Summary | System agent — not user visible | | | | |
| Title | System agent — not user visible | | | | |
...
| ID | Criterion | Test Method |
|----|-----------|-------------|
| AC-001 | `Agent::agent_type()` returns correct `AgentType` for each agent | Unit test |
| AC-002 | `Agent::system_prompt()` returns non-empty string | Unit test |
| AC-003 | `Agent::execute()` returns `AgentResponse` with content or tool calls | Unit test |
| AC-004 | All agent types implement `Sealed` trait | Compile-time check |

### AgentRuntime

| ID | Criterion | Test Method |
|----|-----------|-------------|
| AC-010 | `AgentRuntime::new()` creates runtime with correct session and agent type | Unit test |
| AC-011 | `run_loop()` completes without panic for valid agent/provider/tools | Integration test |
| AC-012 | `invoke_subagent()` delegates task and returns result | Integration test |
| AC-013 | Runtime emits `AgentEvent` during execution | Event listener test |

### Task Delegation

| ID | Criterion | Test Method |
|----|-----------|-------------|
| AC-020 | Task delegate can track multiple tasks simultaneously | Unit test |
| AC-021 | `TaskStatus::Completed` correctly signals completion | Unit test |
| AC-022 | Delegation errors are properly propagated as `DelegationError` | Unit test |

### Event Emission

| ID | Criterion | Test Method |
|----|-----------|-------------|
| AC-030 | `BroadcastEventEmitter` delivers events to all subscribers | Unit test |
| AC-031 | Agent events contain correct session_id and agent_type | Unit test |

### Agent Selection (CLI)

| ID | Criterion | Test Method |
|----|-----------|-------------|
| AC-040 | `--agent build` selects BuildAgent | CLI integration test |
| AC-041 | `--agent plan` selects PlanAgent | CLI integration test |
| AC-042 | Invalid agent type returns error | CLI integration test |
...
|-----------|-------------|
| [Agent System PRD](../../system/02-agent-system.md) | Agent architecture, agent types |
| [Glossary: Agent](../../system/01_glossary.md#agent) | Agent terminology |
| [Glossary: Session](../../system/01_glossary.md#session) | Session referenced by agent |
| [tool.md](../tool.md) | Tool execution within agent loop |
| [provider.md](../provider.md) | LLM provider for agent |
