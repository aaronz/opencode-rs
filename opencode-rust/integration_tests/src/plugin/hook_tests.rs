...
fn test_plugin_hook_execution_on_init() {
    ...
    let plugin_instance = manager
        .get_plugin("test-init-plugin")
...
fn test_plugin_hook_on_tool_call_execution() {
    ...
    let result = manager.on_tool_call_all(
        "read",
...
fn test_plugin_manager_shutdown_all() {
    ...
    manager
        .register(Box::new(plugin1))
        .expect("Plugin 1 should register");
    manager
        .register(Box::new(plugin2))
        .expect("Plugin 2 should register");
...
fn test_plugin_sorted_execution_order() {
    ...
    manager
        .register(Box::new(plugin1))
        .expect("First plugin should register");
    manager
        .register(Box::new(plugin2))
        .expect("Second plugin should register");
...
fn test_plugin_hook_failure_is_contained() {
    ...
    assert!(
        result.is_ok(),
        "Hook execution should succeed even if one plugin fails"
    );
}
...
fn test_plugin_multiple_plugins_all_hooks() {
    ...
    manager
        ...
        .expect("Plugin 2 should register");
    manager
        .register(Box::new(plugin3))
        .expect("Plugin 3 should register");
    ...
    manager
        .on_message_all("Test", "session-123")
        .expect("on_message should succeed");
    manager
        .on_tool_call_all("test_tool", &serde_json::json!({}), "session-123")
        .expect("on_tool_call should succeed");
    manager
        .on_session_end_all("session-123")
...
fn test_plugin_hook_execution_deterministic_order() {
    ...
    manager.on_start_all().expect("on_start_all should succeed");
    manager
        .on_tool_call_all("test", &serde_json::json!({}), "s1")
