...
fn bench_llm_first_token(c: &mut Criterion) {
...
fn bench_llm_full_response(c: &mut Criterion) {
    let mut group = c.benchmark_group("llm_roundtrip");
    group.measurement_time(Duration::from_secs(5));
    group.sample_size(100);
...
fn bench_provider_openai(c: &mut Criterion) {
    let mut group = c.benchmark_group("llm_roundtrip");
    group.measurement_time(Duration::from_secs(5));
...
fn bench_provider_anthropic(c: &mut Criterion) {
    let mut group = c.benchmark_group("llm_roundtrip");
    group.measurement_time(Duration::from_secs(5));
    group.sample_size(50);
...
mod tests {
    ...
    #[test]
    fn test_bench_functions_exist() {
        let empty_fn = || {};
        assert!(std::mem::size_of_val(&empty_fn) > 0);
    }

    #[tokio::test]
    async fn test_mock_streaming_provider() {
        let chunks = vec!["hello".to_string(), " ".to_string(), "world".to_string()];
        let provider = create_mock_streaming_provider(5, 1, chunks, "test-model");
    ...
    #[tokio::test]
    async fn test_mock_streaming_provider_streaming() {
        let chunks = vec!["a".to_string(), "b".to_string(), "c".to_string()];
        let provider = create_mock_streaming_provider(0, 0, chunks, "test-model");

        let collected = Arc::new(Mutex::new(Vec::new()));
        let collected_clone = collected.clone();

        let callback: Box<dyn FnMut(String) + Send + Sync> = Box::new(move |chunk: String| {
    ...
    #[test]
    fn test_create_mock_streaming_provider() {
        let provider = create_mock_streaming_provider(10, 5, vec!["test".to_string()], "model");
        assert_eq!(provider.provider_name(), "mock_streaming");
        assert_eq!(provider.get_models().len(), 1);
    }

    #[test]
    fn test_provider_manager_creates_openai() {
        let manager = ProviderManager::new();
        let spec = ProviderSpec::OpenAI {
        ...
        let provider = manager.create_provider(&spec);
        assert!(provider.is_ok());
    }

    #[test]
    fn test_provider_manager_creates_anthropic() {
        let manager = ProviderManager::new();
        let spec = ProviderSpec::Anthropic {
            api_key: "test".to_string(),
