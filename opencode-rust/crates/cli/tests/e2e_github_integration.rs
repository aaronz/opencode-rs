...
fn test_github_repo_list_requires_token() {
    let output = Command::new("cargo")
        .args(&["run", "-q", "--bin", "opencode-rs", "--", "git-hub", "repo-list"])
...
fn test_github_issue_list_requires_token() {
    let output = Command::new("cargo")
        .args(&["run", "-q", "--bin", "opencode-rs", "--", "git-hub", "issue-list", "owner/repo"])
        .env_remove("GITHUB_TOKEN")
        .output()
        .expect("Failed to execute command");
...
fn test_github_issue_list_invalid_repo_format() {
    let output = Command::new("cargo")
        .args(&["run", "-q", "--bin", "opencode-rs", "--", "git-hub", "issue-list", "invalid-repo"])
...
fn test_github_help_shows_all_commands() {
    let output = Command::new("cargo")
        .args(&["run", "-q", "--bin", "opencode-rs", "--", "git-hub", "--help"])
        .output()
        .expect("Failed to execute command");
    ...
    assert!(
        combined.contains("login") && combined.contains("repo-list") && combined.contains("issue-list"),
...
fn test_github_login_starts_oauth_flow() {
    let output = Command::new("cargo")
        .args(&["run", "-q", "--bin", "opencode-rs", "--", "git-hub", "login"])
        .env_remove("GITHUB_TOKEN")
        .output()
        .expect("Failed to execute command");
...
fn test_github_install_requires_owner() {
    let output = Command::new("cargo")
        .args(&["run", "-q", "--bin", "opencode-rs", "--", "git-hub", "install", "--repo", "myrepo"])
        .env("GITHUB_TOKEN", "test-token")
        .output()
        .expect("Failed to execute command");
...
fn test_github_workflow_requires_owner() {
    let output = Command::new("cargo")
        .args(&["run", "-q", "--bin", "opencode-rs", "--", "git-hub", "workflow", "--repo", "myrepo"])
        .env("GITHUB_TOKEN", "test-token")
        .output()
        .expect("Failed to execute command");
...
fn test_github_integration_repo_list_filters_pull_requests() {
    ...
    let output = Command::new("cargo")
        .args(&["run", "-q", "--bin", "opencode-rs", "--", "git-hub", "issue-list", "test-owner/test-repo"])
        .env("GITHUB_TOKEN", "test-token")
...
fn test_github_integration_returns_api_error() {
    ...
    let output = Command::new("cargo")
        .args(&["run", "-q", "--bin", "opencode-rs", "--", "git-hub", "issue-list", "nonexistent/repo"])
        .env("GITHUB_TOKEN", "test-token")
...
fn test_github_install_with_token_flag() {
    ...
    let output = Command::new("cargo")
        .args(&[
            "run", "-q", "--bin", "opencode-rs", "--", "git-hub", "install",
            "--token", "test-token",
            "--owner", "test-owner",
            "--repo", "test-repo"
        ])
    ...
    assert!(
        combined.contains("workflow") || combined.contains("Workflow") || combined.contains("OpenCode"),
...
fn test_github_workflow_generates_yaml() {
    let output = Command::new("cargo")
        .args(&[
            "run", "-q", "--bin", "opencode-rs", "--", "git-hub", "workflow",
            "--owner", "myorg",
            "--repo", "myrepo",
            "--branch", "develop"
        ])
