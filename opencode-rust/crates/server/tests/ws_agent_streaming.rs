...
#[tokio::test]
async fn test_ws_bidirectional_client_sends_run_receives_response() {
    let (ws_url, server_handle, state_data) = start_ws_test_server_with_state(0).await;

    let ws_url_with_session = ws_url.replace("/test-session", "?session_id=bidirectional-test");
    ...
    ws.send(WsMessage::Text(run_json.to_string()))
        .await
        .expect("Should send run message");
    ...
    while let Some(msg) = ws.next().await {
        if let Ok(text) = msg.unwrap().into_text() {
            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&text) {
                ...
                if received_session_update && received_message {
                    break;
                }
            }
        }
    ...
    assert!(
        received_session_update,
        "Should receive session_update after run"
    );
    assert!(
        received_message,
        "Should receive message response after run"
    );
...
#[tokio::test]
async fn test_ws_bidirectional_client_sends_ping_receives_heartbeat() {
    let (ws_url, server_handle, _state_data) = start_ws_test_server_with_state(0).await;

    let ws_url_with_session = ws_url.replace("/test-session", "?session_id=ping-test");
    ...
    ws.send(WsMessage::Text(ping_json.to_string()))
        .await
    ...
    assert!(received_heartbeat, "Should receive heartbeat after ping");
...
#[tokio::test]
async fn test_ws_bidirectional_same_connection_for_send_and_receive() {
    let (ws_url, server_handle, _state_data) = start_ws_test_server_with_state(0).await;

    let ws_url_with_session = ws_url.replace("/test-session", "?session_id=same-conn-test");
    ...
    assert!(
        message_count >= 3,
        "Should receive multiple responses on same connection"
    );
