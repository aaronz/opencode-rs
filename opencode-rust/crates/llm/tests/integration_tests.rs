...
fn test_integration_multiple_providers_different_types() {
    ...
    let providers = vec![
        ("openai", "gpt-4o", ProviderSpec::OpenAI { api_key: "key".to_string(), model: "gpt-4o".to_string(), base_url: None }),
        ("anthropic", "claude-3-5-sonnet", ProviderSpec::Anthropic { api_key: "key".to_string(), model: "claude-3-5-sonnet".to_string(), base_url: None }),
        ("google", "gemini-1.5-pro", ProviderSpec::Google { api_key: "key".to_string(), model: "gemini-1.5-pro".to_string() }),
    ];
    
    for (provider_type, expected_model, spec) in providers {
        let provider = manager.create_provider(&spec).unwrap();
        assert_eq!(provider.provider_name(), provider_type, "Provider type mismatch for {}", provider_type);
        let models = provider.get_models();
        assert!(!models.is_empty(), "No models for {}", provider_type);
        assert_eq!(models[0].id, expected_model, "Model mismatch for {}", provider_type);
    }
}
...
fn test_integration_model_selection_precedence_explicit_override() {
    ...
    assert_eq!(model, "gpt-4o-mini", "Explicit override should take highest precedence");
}

#[test]
fn test_integration_model_selection_precedence_user_config_provider() {
    let mut config = UserModelConfig::default();
    config.provider_defaults.insert("openai".to_string(), "gpt-4o-mini".to_string());
    
    let selection = ModelSelection::with_user_config(ProviderType::OpenAI, config);
    
    let model = selection.resolve_model(None);
    
    assert_eq!(model, "gpt-4o-mini", "User config provider default should win over global");
}
...
fn test_integration_model_selection_precedence_user_config_global() {
    ...
    assert_eq!(model, "gpt-4o", "Global default should be used when no provider default");
}
...
fn test_integration_model_selection_precedence_provider_default() {
    ...
    assert_eq!(model, "gpt-4o", "Provider default should be used when no user config");
}
...
fn test_integration_model_selection_precedence_full_chain() {
    ...
    config.provider_defaults.insert("openai".to_string(), "fallback-provider".to_string());
    ...
    config_no_override.provider_defaults.insert("openai".to_string(), "fallback-provider".to_string());
    let selection_no_override = ModelSelection::with_user_config(ProviderType::OpenAI, config_no_override);
    assert_eq!(
...
fn test_integration_model_selection_precedence_all_providers() {
    ...
    for (provider_type, expected_default) in providers {
        ...
        assert_eq!(
            ...
            format!("{:?}", provider_type), expected_default
        );
    }
}
...
fn test_integration_local_provider_manager_has_local_providers() {
    ...
    assert!(manager.has_provider("ollama"), "Manager should have ollama provider");
    assert!(manager.has_provider("lmstudio"), "Manager should have lmstudio provider");
    assert!(manager.has_provider("local"), "Manager should have local provider");
}
...
fn test_integration_local_provider_lmstudio_via_manager() {
    ...
    let result = manager.create_provider(&spec);
    assert!(result.is_ok(), "Should create lmstudio provider successfully");
    let provider = result.unwrap();
    assert_eq!(provider.provider_name(), "lmstudio");
}
...
fn test_integration_local_provider_local_inference_via_manager() {
    ...
    let result = manager.create_provider(&spec);
    assert!(result.is_ok(), "Should create local inference provider successfully");
    let provider = result.unwrap();
...
async fn test_integration_local_provider_ollama_complete_fails_gracefully() {
    ...
    assert!(result.is_err(), "Ollama complete should fail without server");
}
...
fn test_integration_provider_spec_serialization_roundtrip() {
    let specs = vec![
        ProviderSpec::OpenAI { api_key: "key".to_string(), model: "gpt-4o".to_string(), base_url: Some("https://api.openai.com".to_string()) },
        ProviderSpec::Anthropic { api_key: "key".to_string(), model: "claude-3-5-sonnet".to_string(), base_url: None },
        ProviderSpec::Google { api_key: "key".to_string(), model: "gemini-1.5-pro".to_string() },
        ProviderSpec::Ollama { base_url: Some("http://localhost:11434".to_string()), model: "llama3".to_string() },
        ProviderSpec::LmStudio { base_url: Some("http://localhost:1234".to_string()), model: "llama3".to_string() },
        ProviderSpec::LocalInference { base_url: "http://localhost:8080".to_string(), model: "llama3".to_string() },
    ];
...
fn test_integration_provider_manager_set_and_get_default() {
    ...
    assert!(manager.get_default().is_none(), "Should have no default initially");
    
    manager.set_default("openai");
    assert_eq!(manager.get_default(), Some("openai"), "Default should be set to openai");
    
    manager.set_default("anthropic");
    assert_eq!(manager.get_default(), Some("anthropic"), "Default should be updated to anthropic");
}
...
fn test_integration_provider_manager_list_providers() {
    ...
    assert!(providers.contains(&"openai".to_string()), "Should contain openai");
    assert!(providers.contains(&"anthropic".to_string()), "Should contain anthropic");
    assert!(providers.contains(&"google".to_string()), "Should contain google");
    assert!(providers.contains(&"ollama".to_string()), "Should contain ollama");
    assert!(providers.contains(&"lmstudio".to_string()), "Should contain lmstudio");
}
...
fn test_integration_reasoning_budget_for_all_providers() {
    ...
    assert_eq!(ReasoningBudget::from_str("invalid"), None, "Invalid budget should return None");
}
...
fn test_integration_reasoning_budget_for_anthropic_provider() {
    ...
    assert!(config.is_some(), "Anthropic should support reasoning budget");
    match config.unwrap() {
        opencode_llm::ProviderReasoningConfig::Anthropic { thinking } => {
            assert!(thinking.is_some(), "Anthropic thinking config should be set for High budget");
        }
...
fn test_integration_user_config_model_preferences() {
    ...
    let anthropic_selection = ModelSelection::with_user_config(ProviderType::Anthropic, config.clone());
    ...
    assert_eq!(ollama_selection.resolve_model(None), "gpt-4o", "Should use global default for unknown provider");
}