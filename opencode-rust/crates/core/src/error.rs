...
impl OpenCodeError {
    /// Returns the numeric error code per FR-118 specification.
    ///
    /// | Range | Category |
    /// |-------|----------|
    /// | 1xxx  | Authentication |
    ...
    pub fn code(&self) -> u16 {
        match self {
            ...
            // Legacy variants mapped to closest range
            Self::Network(_) => 9001,
            Self::Parse(_) => 7003,
            Self::Config(_) => 6002,
            Self::Session(_) => 5001,
    ...
    pub fn http_status(&self) -> u16 {
        let code = self.code();
        match code {
            1000..=1999 => 401, // Authentication → 401
            2000..=2999 => 403, // Authorization → 403
            3000..=3999 => 404, // Provider → 404
            4000..=4999 => 400, // Tool → 400
            5000..=5999 => 404, // Session → 404
            6000..=6999 => 422, // Config → 422
            7000..=7999 => 422, // Validation → 422
            _ => 500,           // Internal → 500
        }
    }
    ...
    /// Converts this error to a unified API response format (FR-118.10).
    ///
    /// Returns `{ "error": { "code": NNNN, "message": "...", "detail": "..." } }`
    /// The `detail` field is only included when `debug_mode` is true.
