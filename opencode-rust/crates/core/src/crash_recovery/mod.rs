//! Crash recovery via panic hooks and crash dumps.
//!
//! This module provides [`CrashRecovery`] for capturing session state
//! when the application crashes unexpectedly.
//!
//! # Crash Dump Strategy
//!
//! When a panic occurs, the crash recovery system:
//! 1. Captures the current session state (messages, tool invocations)
//! 2. Writes a crash dump to `~/.opencode/sessions/crashes/`
//! 3. Includes panic message and stack trace if available
//!
//! Crash dumps can be used to:
//! - Restore sessions after crashes
//! - Debug panics and unexpected termination
...
//! // ... session work ...
//! // If panic occurs, crash dump is automatically written
//! ```
...
pub use types::{
    ActiveSession, CrashDump, CrashRecoveryError, MessageSummary, ToolInvocationSummary,
};
    ...
    pub fn recover_session(&self, path: &PathBuf) -> Result<Session, CrashRecoveryError> {
        ...
        let messages: Vec<Message> = dump
            ...
            .map(|summary| {
                ...
                Message::new(role, summary.content_preview.clone())
            })
            .collect();
