...
fn test_copilot_oauth_success_shows_connect_model_dialog() {
    ...
    assert!(
        ...
        "ConnectModelDialog should be shown after Copilot OAuth success"
    );
    assert!(
        app.pending_copilot_session.is_some(),
        "Copilot session should be stored"
    );
}
...
fn test_copilot_oauth_passes_credentials_to_model_dialog() {
    ...
    assert_eq!(
        stored_session.access_token, expected_token,
        "Access token should be passed to model dialog state"
    );
    assert_eq!(
        app.pending_browser_models.len(),
        2,
        "Model list should be passed to dialog state"
    );
...
fn test_copilot_oauth_model_selection_transitions_to_chat() {
    ...
    assert_eq!(
        app.mode,
        AppMode::Chat,
        "Should transition to Chat after model selection"
    );
    assert!(
        app.connect_model_dialog.is_none(),
        "Model dialog should be closed"
    );
    assert!(
...
fn test_copilot_oauth_sets_copilot_provider_after_model_selection() {
    ...
    assert_eq!(
        ...
        "Provider should be copilot after OAuth flow"
    );
}
...
fn test_copilot_oauth_session_persists_across_model_selection() {
    ...
    assert!(
        app.pending_copilot_session.is_none(),
        "Session should be consumed after model selection"
    );
}
