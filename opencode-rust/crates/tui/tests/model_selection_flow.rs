...
fn test_api_key_validation_transitions_to_connect_model() {
    ...
    assert_eq!(app.pending_api_key_for_provider, Some("sk-ant-valid-key".to_string()), "API key should be preserved");
}
...
fn test_invalid_api_key_shows_error_not_model_dialog() {
    ...
    assert!(app.connect_model_dialog.is_none(), "ConnectModelDialog should NOT be shown after failed validation");
}