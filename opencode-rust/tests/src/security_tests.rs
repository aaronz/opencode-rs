...
fn create_test_pool() -> (StoragePool, tempfile::TempDir) {
    ...
    tokio::runtime::Handle::current().block_on(manager.migrate()).expect("Failed to run migrations");
    (pool, temp_dir)
}
...
async fn test_sql_injection_session_id_handling() {
    ...
    assert!(result.is_ok(), "SQL injection should not crash the repository");
...
async fn test_sql_injection_union_attack() {
    ...
    assert!(result.is_ok());
    assert!(result.unwrap().is_none(), "Union injection should return None");
}
...
async fn test_sql_injection_find_all_with_limit() {
    ...
    assert!(result.is_ok(), "SQL injection in find_all should be handled safely");
...
async fn test_path_traversal_read_outside_project() {
    ...
    let traversal_paths = vec!["/etc/passwd", "../../../etc/passwd", "/root/.ssh/id_rsa", "subdir/../../secrets.txt"];
    for malicious_path in traversal_paths {
        let result = tool.execute(serde_json::json!({"path": malicious_path}), None).await;
        if result.is_ok() {
            let tool_result = result.unwrap();
            assert!(!tool_result.success || !tool_result.content.contains("root:") || !tool_result.content.contains("ssh-rsa"),
                "Path traversal should not read sensitive files: {}", malicious_path);
        }
    }
}
...
async fn test_path_traversal_write_outside_project() {
    ...
    let tool = WriteTool;
    let malicious_paths = vec!["/tmp/payload.txt", "../../../tmp/payload.txt", "/var/www/html/backdoor.php"];
    for malicious_path in malicious_paths {
        let result = tool.execute(serde_json::json!({"path": malicious_path, "content": "malicious content"}), None).await;
        if result.is_ok() {
            let tool_result = result.unwrap();
            if tool_result.success {
                ...
                if file_path.exists() {
                    ...
                    panic!("Write tool should not allow writing outside project: {}", malicious_path);
                }
            }
        }
    }
...
async fn test_path_traversal_null_byte_injection() {
    ...
    for malicious_path in null_byte_paths {
        let result = tool.execute(serde_json::json!({"path": malicious_path}), None).await;
        match result {
            Ok(r) => assert!(!r.success || !r.content.contains("root:"), "Null byte injection should not read sensitive files"),
...
async fn test_session_message_content_sanitization() {
    ...
    session.add_message(opencode_core::message::Message::user(malicious_content.to_string()));
    let json_export = session.export_json();
    assert!(json_export.is_ok());
    let json = json_export.unwrap();
    assert!(!json.contains("DROP TABLE"), "SQL injection in message content should be sanitized in export");
    assert!(json.contains(malicious_content), "Original content should still be preserved");
}
...
async fn test_session_message_xss_prevention() {
    ...
    session.add_message(opencode_core::message::Message::user(xss_content.to_string()));
    let json_export = session.export_json();
    assert!(json_export.is_ok());
    let md_export = session.export_markdown();
    assert!(md_export.is_ok());
    let md = md_export.unwrap();
    assert!(!md.contains("<script>"), "XSS content should be handled safely in markdown export");
}
...
async fn test_tool_registry_execute_rejects_dangerous_paths() {
    ...
    let dangerous_paths = vec!["/etc/shadow", "../../../root/.ssh/id_rsa", "C:\\Windows\\System32\\config\\SAM"];
    for dangerous_path in dangerous_paths {
        let result = registry.execute("read", serde_json::json!({"path": dangerous_path}), None).await;
        match result {
            Ok(r) => {
                if r.success {
                    assert!(!r.content.contains("root:") && !r.content.contains("BEGIN RSA"), "Should not read sensitive files: {}", dangerous_path);
                }
            }
...
async fn test_session_id_format_validation() {
    let (pool, _temp_dir) = create_test_pool();
    let session_repo = Arc::new(SqliteSessionRepository::new(pool.clone()));
    let invalid_ids = vec!["", "   ", "id with spaces", "id\nwith\nnewlines", "id\twith\ttabs"];
    for invalid_id in invalid_ids {
        let result = session_repo.find_by_id(invalid_id).await;
        assert!(result.is_ok(), "Invalid ID should be handled gracefully: {:?}", invalid_id);
        assert!(result.unwrap().is_none());
    }
}
...
async fn test_bash_injection_prevention() {
    ...
    if tool_names.contains(&"bash") {
        ...
        let malicious_commands = vec!["cat /etc/passwd", "echo 'hello'; rm -rf /", "$(cat test.txt)"];
        for cmd in malicious_commands {
            let result = bash_tool.execute(serde_json::json!({"command": cmd, "cwd": project.path().to_string_lossy()}), None).await;
            match result {
                Ok(r) => {
                    if r.success {
                        assert!(!r.content.contains("root:"), "Bash injection should be prevented");
                    }
                }
    ...
    if tool_names.contains(&"grep") {
        ...
        for pattern in malicious_patterns {
            ...
            match result {
                Ok(r) => {
                    if r.success {
                        assert!(!r.content.contains("root:"), "Grep injection should be prevented");
                    }
                }
...
async fn test_write_tool_path_validation() {
    let project = TempProject::new();
    let tool = WriteTool;
    let inside_path = project.path().join("inside.txt").to_string_lossy().to_string();
    let write_attempts = vec![("/tmp/outside.txt", false), ("../outside.txt", false), (inside_path.as_str(), true)];
    for (path, should_succeed) in write_attempts {
        let result = tool.execute(serde_json::json!({"path": path, "content": "test content"}), None).await;
        if should_succeed {
            assert!(result.is_ok() && result.as_ref().unwrap().success, "Write within project should succeed: {}", path);
        }
    }
}
...
async fn test_tool_args_json_injection() {
    let registry = ToolRegistry::new();
    registry.register(ReadTool::new()).await;
    let malicious_args = vec![r#"{"path": "/etc/passwd", "offset": 0}"#, r#"{"path": "/etc/passwd"}"#, r#"{"path": "test.txt", "limit": "0; DROP TABLE"}"#];
    for args_str in malicious_args {
        ...
        if let Ok(args) = args {
            ...
            if result.is_ok() {
                let r = result.unwrap();
                if r.success {
                    assert!(!r.content.contains("root:"), "JSON injection should not expose system files");
                }
            }
        }
    }
...
async fn test_read_tool_symlink_handling() {
    ...
    #[cfg(unix)]
    {
        ...
        let tool = ReadTool::new();
        let result = tool.execute(serde_json::json!({"path": symlink_path.to_string_lossy()}), None).await;
...
async fn test_concurrent_session_access_isolation() {
    ...
    session1.add_message(opencode_core::message::Message::user("Session 1 message".to_string()));
    session_repo.save(&session1).await.ok();
    let session1_id = session1.id.to_string();
    let mut session2 = Session::new();
    session2.add_message(opencode_core::message::Message::user("Session 2 message".to_string()));
    session_repo.save(&session2).await.ok();
    let session2_id = session2.id.to_string();
    let (result1, result2) = tokio::join!(session_repo.find_by_id(&session1_id), session_repo.find_by_id(&session2_id));
    assert!(result1.is_ok());
    assert!(result2.is_ok());
    let loaded1 = result1.unwrap().unwrap();
    let loaded2 = result2.unwrap().unwrap();
    assert!(loaded1.messages.iter().any(|m| m.content.contains("Session 1")), "Session 1 should contain its own message");
    assert!(!loaded1.messages.iter().any(|m| m.content.contains("Session 2")), "Session 1 should not contain Session 2's message");
    assert!(loaded2.messages.iter().any(|m| m.content.contains("Session 2")), "Session 2 should contain its own message");
    assert!(!loaded2.messages.iter().any(|m| m.content.contains("Session 1")), "Session 2 should not contain Session 1's message");
}
...
async fn test_regex_dos_prevention_in_tools() {
    ...
    if tool_names.contains(&"grep") {
        ...
        for pattern in catastrophic_patterns {
            let result = grep_tool.execute(serde_json::json!({"path": "test_file.txt", "pattern": pattern}), None).await;
            match result {
                Ok(r) => assert!(!r.content.contains("ReDoS"), "Tool should handle or reject ReDoS patterns"),
...
async fn test_sql_injection_sessions() {
    ...
    let session_repo = Arc::new(SqliteSessionRepository::new(pool.clone()));
    let mut session = Session::new();
    session.add_message(opencode_core::message::Message::user("Test session for SQL injection".to_string()));
    ...
    let sql_injection_payloads = vec!["'; DROP TABLE sessions; --", "' OR '1'='1", "' UNION SELECT * FROM sessions--", "1'; DROP TABLE sessions; --", "admin'--", "' OR 1=1--"];
    for payload in sql_injection_payloads {
        let result = session_repo.find_by_id(payload).await;
        assert!(result.is_ok(), "SQL injection payload '{}' should not crash the repository", payload);
        assert!(result.as_ref().unwrap().is_none(), "SQL injection payload should return None");
    }
    ...
    assert!(legitimate_result.is_ok() && legitimate_result.unwrap().is_some(), "The legitimate session should still be accessible");
}
