# OpenCode RS Specification - Iteration 19

**Version:** 1.9.0
**Generated:** 2026-04-14
**Status:** Implementation ~92-96% Complete
...
|-------|----------|--------|--------|--------------|
| Phase 6 Release Qualification not started | P1 | all | ❌ NOT STARTED | End-to-end testing, benchmarks, security audit |
| `desktop_web_different_ports` test failing | P1 | cli | ❌ NOT FIXED | Use dynamic port allocation instead of hardcoded port 3000 |
| `test_bedrock_credential_resolution_bearer_token_priority` fails with `--all-features` | P2 | llm | ❌ NOT FIXED | Test pollution from AWS env vars - needs test isolation |
| Trailing whitespace in `storage/src/service.rs` | P2 | storage | ❌ NOT FIXED | Run `cargo fmt` to fix whitespace |
| Deprecated `mode` field | P2 | config | ⚠️ Deferred | Marked for removal in v4.0 |
| Deprecated `tools` field | P2 | config | ⚠️ Deferred | Marked for removal after migration |

### 18.2 desktop_web_different_ports Test
...
**Fix Required:** Use dynamic port allocation:
```rust
let listener = TcpListener::bind("127.0.0.1:0")?;
let port = listener.local_addr()?.port();
```

### 18.3 Bedrock Credential Test Pollution
...
**Fix Required:** Use `temp_env` or similar pattern to isolate environment variables, or run this test in a separate process.
...
- 1-2 failures depending on test configuration

Failed tests breakdown:
- CLI tests: 1 failure (desktop_web_different_ports - port conflict)
- LLM tests: 1 conditional failure (bedrock credential test - only fails with --all-features due to env pollution)

Fixed since iteration-18:
- GitLab CI integration tests now use mock server - all 7 tests passing
```

---

## 21. Iteration 19 Changes

### Improvements Since Iteration-18
...
3. **Phase 4/5 Completion Increased**
   - Phase 4: ~95% → ~98%
   - Phase 5: ~95% → ~98%

### Remaining Work

1. **P1 - Fix `desktop_web_different_ports` Test**
