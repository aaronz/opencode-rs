# OpenCode Rust Port — Specification Document v10

**Version:** 10.0
**Generated:** 2026-04-13
**Based on:** PRD specifications and gap analysis (Iteration 10)
**Status:** Draft
...
| TD-016 | **Clippy errors (18)** | **core, ratatui-testing** | **HIGH** | **Fix all clippy errors** | **P0-9 OPEN** |
...
| **P0-9** | **Clippy fails with `-D warnings` (18 errors)** | **core, ratatui-testing** | **n/a** | ❌ **OPEN** |

**P0 Blockers Summary:** 1 remaining (P0-9: 18 clippy errors) - **UNCHANGED FROM ITERATION 9**
...
| Error | File | Count | Fix |
|-------|------|-------|-----|
| deprecated `AgentMode` enum | config.rs:436 | 2 | Remove or use `permission` field |
| deprecated `AgentConfig::mode` field | command.rs:567, config.rs:2771 | 2 | Remove or use `permission` field |
| `question_mark` (use `?`) | config.rs:1594 | 1 | Rewrite block with `?` operator |
| `needless_borrows_for_generic_args` | config.rs:2068 | 1 | Remove unnecessary borrow |
| `redundant_closure` (use `ok_or`) | session_sharing.rs:323 | 1 | Use `ok_or()` instead of closure |
| `map_entry` (use entry API) | session_sharing.rs:225 | 1 | Use `entry()` API properly |
| `and_then` → `map` | crash_recovery.rs:241 | 1 | Replace with `map` |
| `very_complex_type` | skill.rs | 1 | Factor into type alias |
| `&PathBuf` → `&Path` | skill.rs:116 | 5 | Change to `&Path` |
...
| P0-new-3 ACP HTTP+SSE transport | ✅ Done | - | IMPLEMENTED |
...
| Iteration 9 Spec | [spec_v9.md](./spec_v9.md) | Previous version |
| Iteration 8 Spec | [spec_v8.md](./spec_v8.md) | Earlier version |
