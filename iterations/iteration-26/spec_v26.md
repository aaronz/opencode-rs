# ratatui-testing Specification v2.6
...
**FR-LIB-001**: All core types must be exported from `lib.rs` for public API access
...
- Windows: Stub implementation returning errors (`anyhow::bail!("PTY not supported on Windows")`)
...
**Known Limitations**:
- Windows: PTY operations return errors (Windows PTY support is best-effort per PRD)
...
**FR-DIFF-007**: `diff_str(expected, actual)` parses strings to Buffers then diffs
...
**Known Issues**:
- **FR-DIFF-GAP-001**: `diff_str()` does not apply IgnoreOptions - string comparison ignores builder options
...
**FR-DSL-004**: `with_pty(command)` initializes PTY with command
...
impl CliTester {
    ...
    pub fn capture_stdout(mut self) -> Self;  // FR-CLI-015
    pub fn capture_stderr(mut self) -> Self;  // FR-CLI-016
...
impl ChildProcess {
    pub async fn wait(mut self) -> Result<CliOutput>;  // FR-CLI-017
    pub async fn kill(&mut self) -> Result<()>;  // FR-CLI-018
}
...
**FR-CLI-007**: `with_temp_dir()` creates temp directory that is cleaned up on drop; returns `(Self, PathBuf)` tuple
...
**FR-CLI-015**: `capture_stdout()` enables stdout capture (fluent)

**FR-CLI-016**: `capture_stderr()` enables stderr capture (fluent)

**FR-CLI-017**: `ChildProcess::wait()` waits for child to complete

**FR-CLI-018**: `ChildProcess::kill()` kills the child process

**Dependencies**: `tokio`, `tempfile`

**Implementation Notes**:
- When `with_temp_dir()` is called without explicit `working_dir()`, the temp directory is used as working directory (lines 100-101 in cli.rs)
- `temp_dir` is automatically cleaned up when `CliTester` is dropped
...
**Known Limitations**:
- **FR-SNAP-GAP-001**: `SNAPSHOT_DIR` is hardcoded to `"snapshots"` - not configurable
...
### 3.1 P1 - High Priority

| Gap ID | Description | Impact | Status |
|--------|-------------|--------|--------|
| FR-DIFF-GAP-001 | `diff_str()` does not apply IgnoreOptions | String comparison ignores builder options | Open |
| FR-SNAP-GAP-001 | Snapshot directory hardcoded to "snapshots" | Inflexible snapshot directory | Open |

### 3.2 P2 - Medium Priority

| Gap ID | Description | Impact | Status |
|--------|-------------|--------|--------|
| FR-PTY-GAP-001 | Windows PTY stub returns errors | Windows PTY testing not supported | Documented limitation |
| FR-DIALOG-GAP-001 | DialogRenderTester tests incomplete | Limited dialog verification coverage | Partial |
...
├── tests/
│   ├── pty_tests.rs        # PtySimulator tests
│   ├── buffer_diff_tests.rs # BufferDiff tests
│   ├── state_tests.rs      # StateTester tests
│   ├── dsl_tests.rs        # TestDsl tests
...
**Note**: `crossterm` is a required dependency (not optional) since PTY event injection depends on it.
...
- [x] is_child_running() check (FR-PTY-008)
- [x] Cross-platform detection (Windows returns error)
...
- [ ] `diff_str` supports IgnoreOptions (FR-DIFF-GAP-001 - OPEN)
...
- [x] All additional methods implemented
...
- [x] `with_pty()` with command parameter (FR-DSL-004)

### CliTester
- [x] Spawns process with args (FR-CLI-008)
- [x] Captures stdout/stderr (FR-CLI-008, FR-CLI-015, FR-CLI-016)
- [x] Returns exit code (FR-CLI-008)
- [x] Cleans up temp directories (FR-CLI-007)
- [x] temp_dir used as working_dir when no explicit dir set (FR-CLI-GAP-001 - FIXED)
- [x] ChildProcess exported (FR-LIB-GAP-001 - FIXED)
...
| Test File | Module | Line Count | Status |
|-----------|--------|------------|--------|
| `tests/pty_tests.rs` | PtySimulator | 363 | Complete |
| `tests/buffer_diff_tests.rs` | BufferDiff | 513 | Complete |
| `tests/state_tests.rs` | StateTester | 592 | Complete |
| `tests/dsl_tests.rs` | TestDsl | 1144 | Complete |
| `tests/dsl_integration_tests.rs` | TestDsl Integration | 650 | Complete |
| `tests/dialog_tests.rs` | DialogRenderTester | 199 | Partial |
| `tests/integration_tests.rs` | All | 647 | Complete |
...
|---------|------|---------|
| v2.6 | 2026-04-17 | Gap analysis update: marked resolved issues, added FR-CLI-015-018, noted crossterm as required, updated test coverage |
