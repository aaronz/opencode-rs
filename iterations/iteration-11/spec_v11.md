# OpenCode Rust Port — Specification Document v11

**Version:** 11.0
**Generated:** 2026-04-13
**Based on:** PRD specifications and gap analysis (Iteration 11)
**Status:** Draft
...
- **Components:**
  ...
  - Custom tool registration via plugin `tool` property ✅
  - Plugin-provided tool execution with context ✅
  - Compaction hook support (`experimental.session.compacting`) ✅
  ...
  - Loading precedence: global config → project config → global plugins → project plugins ✅
...
| P0-8 | Clippy unreachable pattern | permission | n/a | ✅ **RESOLVED** |
| P0-new-2 | Desktop WebView integration | cli | FR-015 | ✅ **RESOLVED** |
| P0-new-3 | ACP HTTP+SSE transport | cli/server | FR-015 | ✅ **RESOLVED** |
| ~~P0-9~~ | ~~Clippy fails with `-D warnings` (18 errors)~~ | ~~core, ratatui-testing~~ | ~~n/a~~ | ✅ **RESOLVED** |

**P0 Blockers Summary:** ✅ **ALL P0 BLOCKERS RESOLVED!**
...
**P1 Flaky Test (New in Iteration 11):**
| ID | Issue | Module | Status |
|----|-------|--------|--------|
| P1-F1 | `test_theme_config_resolve_path_tilde_expansion` fails | core/config | ❌ Failing |
...
| P2-3 | Compaction shareability verification | ✅ Done |
...
|----|------|------|----------|-------------|
| ~~CQ-1~~ | ~~Clippy errors (18 total)~~ | ~~core, ratatui-testing~~ | ~~HIGH~~ | ~~✅ RESOLVED in Iteration 11~~ |
| **CQ-2** | **Flaky test** | **core/config** | **HIGH** | **`test_theme_config_resolve_path_tilde_expansion` fails on macOS** |

### Flaky Test Detail (CQ-2)

| Test | Issue | Fix |
|------|-------|-----|
| `test_theme_config_resolve_path_tilde_expansion` | `dirs::home_dir()` doesn't respect `HOME` env var on macOS | Use `dirs_next::home_dir()` or mock properly |
...
**✅ PASSES** - All 18 clippy errors have been resolved in Iteration 11!
...
|------|--------|-------|-----|
| `test_theme_config_resolve_path_tilde_expansion` | ❌ **FAILING** | `dirs::home_dir()` doesn't respect `HOME` env var on macOS | Use `dirs_next::home_dir()` or mock properly |
...
|-------|-------|-----|--------|-----------|
| `crates/core/` | 1 | `01`, `06` | ✅ Complete | P1-F1 (flaky test), TD-003-009 |
...
| Iteration 10 Spec | [spec_v10.md](./spec_v10.md) | Previous version |
| Iteration 9 Spec | [spec_v9.md](./spec_v9.md) | Earlier version |

---

## 15. Plugin System (PRD 08) - Detailed Specification

### FR-008: Plugin System - Extended

The Plugin System (PRD 08) defines the server/runtime plugin architecture for OpenCode extensions.
...
| `tui` | Runs in the TUI process (see FR-009: TUI Plugin API) |
...
## 16. MCP System (PRD 04) - Detailed Specification

### FR-010: MCP Integration - Extended

The MCP System (PRD 04) defines the Model Context Protocol integration for OpenCode.
...
| Vercel Grep | `type: "remote"`, `url: "https://mcp.grep.app"` |

#### Transport

**Local Servers:** stdio transport with JSON-RPC protocol
...
|---------|------|---------|
| 11.0 | 2026-04-13 | Updated based on Iteration 11 gap analysis. **Overall completion ~92-94%** (up from ~90-92%). **P0-9 RESOLVED** - All 18 clippy errors fixed! Clippy now passes with `-D warnings`. Added P1-F1 for flaky test `test_theme_config_resolve_path_tilde_expansion`. Updated Phase 5 hardening to ~95%, Phase 6 to ~80%. Added FR-010 extended specification with MCP server config, OAuth, tool naming, transport details. All P0 blockers now resolved. Technical debt updated (removed TD-016 for clippy errors, added deprecated fields DEP-3 through DEP-8). Build status now shows all crates passing clippy. |
| 10.0 | 2026-04-13 | Updated based on Iteration 10 gap analysis. Overall completion ~90-92% (unchanged). P0-9 remains the sole P0 blocker (18 clippy errors). No new issues identified. Added detailed Plugin System (FR-008) specification with all event hooks documented. P1-3 continues in progress (deprecated fields). Technical debt unchanged. Phase 5 hardening ~85%. Phase 6 release qualification ~70%. |
| 9.0 | 2026-04-12 | Updated based on Iteration 9 gap analysis. Overall completion ~90-92%. Fixed P0-8 (clippy unreachable pattern), P0-new-2 (Desktop WebView), P1-2 (circular detection), P1-9 (session sharing). Fixed P2-1, P2-2, P2-9, P2-12, P2-13, P2-14, P2-15. All dead code DC-1 through DC-10 cleaned up. New P0-9: 18 clippy errors (17 in core, 1 in ratatui-testing). Phase 5 hardening ~85%. Phase 6 release qualification ~70%. TD-001 and TD-002 resolved. Added TD-016 for clippy errors. |
