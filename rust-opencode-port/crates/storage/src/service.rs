...
impl StorageService {
    ...
    pub async fn save_session(&self, session: &Session) -> Result<(), OpenCodeError> {
        ...
        let json = serde_json::to_string(&model).map_err(|e| OpenCodeError::Storage(e.to_string()))?;
    ...
    pub async fn load_session(&self, id: &str) -> Result<Option<Session>, OpenCodeError> {
        ...
        match res {
            Some(model) => {
                let session = Session::try_from(model).map_err(|e| OpenCodeError::Storage(e.to_string()))?;
                Ok(Some(session))
            },
            None => Ok(None)
        }
    ...
    pub async fn get_session_messages_paginated(
        ...
        let session = self.load_session(id).await?.ok_or_else(|| OpenCodeError::Storage("Session not found".to_string()))?;
    ...
    pub async fn list_sessions(&self, limit: usize, offset: usize) -> Result<Vec<SessionInfo>, OpenCodeError> {
        let conn = self.pool.get().await?;
        
        let models = conn.execute(move |c| {
            let mut stmt = c.prepare(
    ...
    pub async fn load_project_by_path(&self, path: &str) -> Result<Option<ProjectModel>, OpenCodeError> {
        let conn = self.pool.get().await?;
    ...
    pub async fn list_projects(&self, limit: usize, offset: usize) -> Result<Vec<ProjectModel>, OpenCodeError> {
        let conn = self.pool.get().await?;
    ...
    pub async fn load_account_by_id(&self, id: &str) -> Result<Option<AccountModel>, OpenCodeError> {
        let conn = self.pool.get().await?;
        let id_str = id.to_string();
    ...
    pub async fn list_accounts(&self, limit: usize, offset: usize) -> Result<Vec<AccountModel>, OpenCodeError> {
        let conn = self.pool.get().await?;
    ...
    pub async fn get_user_permissions(&self, user_id: &str) -> Result<Vec<(String, bool)>, OpenCodeError> {
        let conn = self.pool.get().await?;
        let user_id_str = user_id.to_string();
        
        conn.execute(move |c| {
            let mut stmt = c.prepare(
                "SELECT permission, is_deny FROM permissions WHERE user_id = ?1"
            )?;
