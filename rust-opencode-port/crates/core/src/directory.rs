...
impl DirectoryScanner {
    ...
    pub fn discover_skills(&self) -> Result<Vec<crate::skill::Skill>, OpenCodeError> {
                ...
                if project_skills_dir.exists() {
                        ...
                        crate::skill::SkillManager::new().discover_in_dir(&project_skills_dir)
                    {
                        let project_names: std::collections::HashSet<String> = project_skills
                            .iter()
                            .map(|s: &crate::skill::Skill| s.name.clone())
                            .collect();

                        skills.retain(|s: &crate::skill::Skill| !project_names.contains(&s.name));
